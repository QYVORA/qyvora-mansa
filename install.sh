...
banner() {
    echo -e "${CYAN}  __  __      __  __  _____ ___    ____    ${NC}"
    echo -e "${CYAN} |  \\/  |\\ \\  / / / / \\__   \\   / ___|   ${NC}"
    echo -e "${CYAN} | |\\/| | \\ \\/ /  \\ \\   | | \\ \\ \\___ \\   ${NC}"
    echo -e "${CYAN} | |  | |  \\  /    \\ \\__| |  \\_|  ___) |  ${NC}"
    echo -e "${CYAN} |_|  |_|   \\/      \\____/  \\____|____/    ${NC}"
    echo -e "${NC}"
    echo -e "  ${WHITE}MANSA — Zero-Config Installer${NC}"
...
install_release() {
    ...
    if [ "$os" != "windows" ]; then
        if curl -fsSL --connect-timeout 10 "${BASE_URL}/mansa.png" -o "${WORK_DIR}/mansa.png"; then
            if [ -n "$hashtool" ] && [ -f "${WORK_DIR}/checksums.txt" ]; then
                local cwant cgot
                cwant=$(awk -v n="mansa.png" '$2 == n { print $1 }' "${WORK_DIR}/checksums.txt")
                if [ -n "$cwant" ]; then
                    cgot=$($hashtool "${WORK_DIR}/mansa.png" | awk '{ print $1 }')
                    if [ "$cwant" = "$cgot" ]; then
                        ok "SHA-256 verified for mansa.png"
                    else
                        warn "Icon checksum mismatch; desktop icon will be skipped."
                        rm -f "${WORK_DIR}/mansa.png"
                    fi
                fi
            fi
        else
            warn "Could not fetch mansa.png; desktop icon will be skipped."
        fi
    fi
...
install_from_source() {
    ...
    if [ -f "$PWD/go.mod" ]; then
        ...
        if CGO_ENABLED=0 go build -trimpath -buildvcs=false -ldflags="-s -w" -o "${WORK_DIR}/mansa-src" ./cmd/mansa; then
            INSTALL_BIN="${WORK_DIR}/mansa-src"
            return 0
        else
    ...
    if ! (cd "${WORK_DIR}/src" && CGO_ENABLED=0 go build -trimpath -buildvcs=false -ldflags="-s -w" -o "${WORK_DIR}/mansa-src" ./cmd/mansa); then
...
install_binary() {
    ...
    if [ "$dest" = "/usr/local/bin" ] && [ ! -w "$dest" ]; then
        if sudo -n true 2>/dev/null; then
            sudo -n install -m 0755 "$INSTALL_BIN" "$dest/mansa"
        else
    ...
    mkdir -p "$dest"
    if [ -w "$dest" ]; then
        install -m 0755 "$INSTALL_BIN" "$dest/mansa"
...
configure_path() {
    ...
    printf '\n# added by MANSA installer\n%s\n' "$line" >> "$rc"
    warn "Added ${dest} to your PATH in ${rc}"
    warn "Run: source ${rc}  (or open a new terminal) to use 'mansa'"
}

# ---------------------------------------------------------------------------
# Desktop integration (Linux only): install the app icon and a .desktop entry
# so mansa shows up with its logo in GNOME's search/app grid. Data goes next
...
install_desktop_integration() {
    ...
    local icon_src="${WORK_DIR}/mansa.png"
    if [ -f "$PWD/assets/mansa.png" ]; then
        icon_src="$PWD/assets/mansa.png"
    elif [ ! -f "$icon_src" ] && [ -f "${WORK_DIR}/src/assets/mansa.png" ]; then
        icon_src="${WORK_DIR}/src/assets/mansa.png"
    fi
    ...
    install -m 0644 "$icon_src" "$icon_dir/mansa.png"

    local desktop="$apps_dir/mansa.desktop"
    if [ ! -f "$desktop" ]; then
        cat >"$desktop" <<EOF
[Desktop Entry]
Type=Application
Name=MANSA
GenericName=Wireless Security Assessment Framework
Comment=Authorized wireless security assessment, WLAN security analysis, and evidence-driven reporting
Exec=$dest/mansa
Icon=$icon_dir/mansa.png
Terminal=true
Categories=Utility;Development;Security;
Keywords=wireless;wlan;wifi;security;assessment;scan;
EOF
    fi
    chmod 0644 "$desktop"
    ok "Installed desktop entry: ${desktop}"
    ok "Installed app icon: ${icon_dir}/mansa.png"
...
verify() {
    local dest="$1"
    local bin="$dest/mansa"

    if [ ! -x "$bin" ]; then
        warn "mansa binary not found at ${bin}."
        return 1
    fi

    info "Verifying installation..."
    "$bin" version 2>/dev/null || "$bin" --help >/dev/null 2>&1
    ok "MANSA installed at ${bin}"
    ok "Run 'mansa --help' to get started."
    return 0
}
